    ...
    socket.onmessage = (event: any) => {
      ...
      if (data.positions) {
        ...
        if (pos && !pos.attributes.result) {
          console.log(pos);
          hub.dispatch(HUB_EVENTS.GISPOSITION, { position: pos }, 'collaudo');
          updatePosition(pos);
        }
        if (pos && pos.attributes.result) {
          ...
          hub.dispatch(HUB_EVENTS.GISRESULT, { position: pos }, 'collaudo');
        }
      }
    ...
    return (
      <div className="p-grid w-full">
        <div className="p-col-12">
          <CollaudoBot
            onCollaudoEnd={props.onCollaudoEnd}
            deviceId={position.deviceId}
            online={gisDeviceStatus === 'online'}
          />
        </div>
